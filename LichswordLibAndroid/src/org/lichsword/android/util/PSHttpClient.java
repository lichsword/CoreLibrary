...
//            "like Gecko) Version/3.1.2 Safari/525.20.1";
    ...
    public Response post(String url) throws Exception {
        return post(url, new HttpParameter[0] ,null);
    }

    public Response post(String url,HttpParameter... postParams) throws Exception {
        return post(url, postParams,null);
    }

    public Response post(String url, HttpParameter[] postParams,HttpParameter[] heads) throws Exception {
    ...
    public Response get(String url, HttpParameter... queryParams) throws Exception {
        return get(url,queryParams,null);
    }

    public Response get(String url, HttpParameter[] queryParams,HttpParameter[] heads) throws Exception {
    ...
    protected Response httpRequest(String url, HttpParameter[] postParams,HttpParameter[] heads) throws Exception {
        int retriedCount;
...
//                log("Sleeping " + retryIntervalMillis +" millisecs for next retry.");
    ...
    public static String encodeParameters(HttpParameter[] postParams) {
        ...
        for (HttpParameter httpParameter : postParams) {
            if (buf.length() > 0) {
                buf.append("&");
            }
            buf.append(URLEncoder.encode(httpParameter.name))
               .append("=")
               .append(URLEncoder.encode(httpParameter.value));
        }
        return buf.toString();
     ...
     */
    private void setHeaders(String url, HttpParameter[] heads, HttpURLConnection connection) {

        connection.setRequestProperty("User-Agent", userAgent);
        if(heads != null){
            for (HttpParameter head : heads) {
    ...
    private static void log(String message) {
        if (DEBUG) {
            LogHelper.d("MgeekHttpClient",message);
        }
    }

    private static void log(String message1,String message2) {
        if (DEBUG) {
            LogHelper.d("MgeekHttpClient",message1 + "\n" + message2);
        }
    }
